...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total corallium balance of the wallet (excluding zerocoins)\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        coralliumObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    coralliumObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("CORALLIUMsupply", coralliumObj));
    */
#ifdef ENABLE_WALLET
    if (pwalletMain) {
...
#ifdef ENABLE_WALLET
        // Case 1: Corallium address and we have full public key:
