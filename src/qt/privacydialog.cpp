                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 CORALLIUM ought to be enough for anybody." - Bill Gates, 2017
    ui->CORALLIUMpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelCORALLIUMSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintCORALLIUM->setEnabled(false);
        ui->pushButtonMintCORALLIUM->setToolTip(tr("CORALLIUM is currently disabled due to maintenance."));

        ui->pushButtonSpendCORALLIUM->setEnabled(false);
        ui->pushButtonSpendCORALLIUM->setToolTip(tr("CORALLIUM is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->CORALLIUMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintCORALLIUM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("CORALLIUM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " CORALLIUM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" CORALLIUM in ") +
...
void PrivacyDialog::on_pushButtonSpendCORALLIUM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("CORALLIUM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn CORALLIUM
        sendCORALLIUM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send CORALLIUM
    sendCORALLIUM();
}
...
void PrivacyDialog::sendCORALLIUM()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Corallium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->CORALLIUMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as CORALLIUM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CRLM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " CORALLIUM</b>";
    ...
    // use mints from zCrlm selector if applicable
    ...
    // Clear zcrlm selector in case it was used
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCrlm Spend #: ") + QString::number(nCount) + ", ";
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->CORALLIUMpayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " CORALLIUM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" CORALLIUM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" CORALLIUM "));
    ui->labelCORALLIUMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelCORALLIUMSyncStatus->setVisible(fShow);
}
