...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCrlm spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zCrlm spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zCrlm spend with serial %s from tx %s is not in valid range\n",
...
{
    int64_t nSubsidy = 0;
    int lastDigitA = nHeight % 10;
    int lastDigitB = nHeight % 1000;

    if (nHeight < 2 && nHeight > 0) {
	nSubsidy = 500000 * COIN;       // Premine 500K Coins
    } else if (nHeight < 10000) {
	if (lastDigitB == 777) {	// Network-Test 
	  nSubsidy = 0.77 * COIN;       // Lucky Seven Block 0.77 Coins
	} else if (lastDigitA == 1) {
	  nSubsidy = 0.15 * COIN;       // Ultra Block 0.15 Coins
	} else if (lastDigitA == 3 || lastDigitA == 6) {
	  nSubsidy = 0.1 * COIN;	// Super Block 0.1 Coins
	} else {
	  nSubsidy = 0.05 * COIN;       // Regular Block 0.05 Coins
	}
    } else {
    	if (lastDigitB == 777) {	// Live Network
    	  nSubsidy = 77 * COIN;         // Lucky Seven Block 77 Coins
    	} else if (lastDigitA == 1) {
    	  nSubsidy = 15 * COIN;         // Ultra Block 15 Coins
    	} else if (lastDigitA == 3 || lastDigitA == 6) {
    	  nSubsidy = 10 * COIN;         // Super Block 10 Coins
    	} else {
    	  nSubsidy = 5 * COIN;          // Regular Block 5 Coins
	}
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < 10000) {
	ret = blockValue * 0.001;
    } else if (nHeight > 90000 && nHeight < 120000) {
        ret = blockValue * 0.7;
    } else if (nHeight > 120000 && nHeight < 135000) {
        ret = blockValue * 0.65;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Corallium
...
void RecalculateZCRLMSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from CORALLIUM supply
...
bool RecalculateCRLMSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    */
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zCrlm with serial %s is already in the block %d\n",
    ...
    if (!fVerifyingBlocks && pindex->nHeight == Params().Zerocoin_StartHeight() + 1) {
        RecalculateZCRLMMinted();
        RecalculateZCRLMSpent();
        RecalculateCRLMSupply(1);
    }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CRLM to CORALLIUM
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCrlm spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCrlm serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCrlm mints and %d zCrlm spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Corallium: We use certain sporks during IBD, so check to see if they are
